...
void
printWeightAsMatrix(const T_ELEM *wDev, const int nRows, const int nCols) {
...
void
cudnnErrCheck_(cudnnStatus_t stat, const char *file, int line) {
...
template <typename T_ELEM>
__global__ void
initGPUData_ker(T_ELEM *data, int numElements, T_ELEM value) {
...
template <typename T_ELEM>
void
initGPUData(T_ELEM *data, int numElements, T_ELEM value) {
    dim3 gridDim;
    dim3 blockDim;
...
class RNNSample {
   public:
    ...
    void
    setup(RNNSampleOptions &options);
...
static void
parseRNNSampleParameters(int argc, char **argv, RNNSampleOptions *options) {
    struct cmdParams {
        const char *name;
        const char *format;
        size_t offset;
        ...
        {"inputMode",
         "%d",
         offsetof(RNNSampleOptions, inputMode),
         "input to the RNN model (0-skip input, 1-linear input)"},
        {"dirMode",
         "%d",
         offsetof(RNNSampleOptions, dirMode),
        ...
        {"biasMode",
         "%d",
         offsetof(RNNSampleOptions, biasMode),
         "bias type (0-no bias, 1-inp bias, 2-rec bias, 3-double bias"},
        {"algorithm",
         "%d",
         offsetof(RNNSampleOptions, algorithm),
         "recurrence algorithm (0-standard, 1-persist static, 2-persist dynamic"},
        {"mathPrecision", "%d", offsetof(RNNSampleOptions, mathPrecision), "math precision (0-FP16, 1-FP32, 2-FP64)"},
        {"mathType",
         "%d",
         offsetof(RNNSampleOptions, mathType),
